...
echo -e "${GREEN}===============================${NC}"
echo -e "${GREEN}Starting READ-ONLY benchmark${NC}"
echo -e "${GREEN}===============================${NC}"
...
USERS=$USERS SPAWN_RATE=$SPAWN_RATE RUN_TIME=$RUN_TIME "$SCRIPT_DIR/run_locust_benchmark.sh" --read-only

# Find the most recent metrics file and log file
METRICS_FILE=$(ls -t benchmark_results/locust_metrics_*.json 2>/dev/null | head -1)
LOG_FILE=$(ls -t benchmark_results/locust_read-only.log 2>/dev/null | head -1)
...
if [ -n "$METRICS_FILE" ]; then
    ...
    if command -v jq &> /dev/null; then
        READ_THROUGHPUT=$(jq '.throughput_ops_sec.read' "$METRICS_FILE")
        AVG_LATENCY=$(jq '.read_latency_ms.avg' "$METRICS_FILE")
        P95_LATENCY=$(jq '.read_latency_ms.p95' "$METRICS_FILE")
        SUCCESS_RATE=$(jq '.success_rate' "$METRICS_FILE")
        TOTAL_OPS=$(jq '.operations.total' "$METRICS_FILE")

        echo -e "Read Throughput: ${GREEN}${READ_THROUGHPUT}${NC} ops/sec"
